 ...
 * ADDING FX Intervention (FXI), but no effective
...
var c ch cf prh rer rnom pic pih lab rw y mchr a g z cstar varphi gammaH dep bstar chstar istar pistar tb fx;
varexo e_a e_g e_z e_cstar e_istar e_pistar e_varphi e_gammaH e_fx;

parameters beta_C sigma alpha_c eta_c sigma_L theta_p phi_i phi_pic phi_y eta_star rho_a rho_g rho_z rho_cstar;
parameters psi_bstar rho_istar rho_pistar rho_varphi rho_gammaH rho_fx;
...
psi_bstar   = 0.001;// 0.001 (No effective FX) or 0.2 (Effective FXI)
...
bstar - fx = 1/(beta_C)*(bstar(-1)-fx(-1)) + alpha_c*(rer+cf) - alpha_c*(prh + cstar-eta_star*(prh-rer)) ;
...
@#if DITR == 1
    ...
    save('results3a_DITR','DITR_oo_','DITR_case_title');
@#else
    @#if CITR ==1
         CITR_oo_ = oo_;
    ...
    @#else
        @#if PEG ==1
             ...
             save('results3a_PEG','PEG_oo_','PEG_case_title');
        @#else
            @#if DITR_F ==1
                 DITR_F_oo_ = oo_;
                 DITR_F_case_title = case_title;
                 save('results3a_DITR_F','DITR_F_oo_','DITR_F_case_title');
            @#else
