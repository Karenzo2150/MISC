 ...
 * ADDING FX Intervention (FXI) and effective
...
psi_bstar   = 0.2;// 0.001 (No effective FX) or 0.2 (Effective FXI)
...
@#if DITR == 1
    ...
    save('results3b_DITR','DITR_oo_','DITR_case_title');
@#else
    @#if CITR ==1
         CITR_oo_ = oo_;
    ...
    @#else
        @#if PEG ==1
             ...
             save('results3b_PEG','PEG_oo_','PEG_case_title');
        @#else
            @#if DITR_F ==1
                 DITR_F_oo_ = oo_;
                 DITR_F_case_title = case_title;
                 save('results3b_DITR_F','DITR_F_oo_','DITR_F_case_title');
            @#else
