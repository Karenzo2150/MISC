 ...
 * in endogenous risk premium depending on foreign debt to 
 * generate stationarity
 * Pau Rabanal (September 2025)
 * Endogenous risk premim: Psi_t = (1 + psi_bstar*Bstar)
 * It includes interest rate smoothing in the Taylor-type rules
 * Different cases for the monetary/exchange rate policy
 * DITR_F: Full stabilization of domestic inflation (4)
 */
...
var c ch cf prh rer rnom pic pih lab rw y mchr a g z cstar varphi gammaH dep bstar chstar istar pistar tb;
varexo e_a e_g e_z e_cstar e_istar e_pistar e_varphi e_gammaH;

parameters beta_C sigma alpha_c eta_c sigma_L theta_p phi_pic phi_y phi_i eta_star rho_a rho_g rho_z rho_cstar;
...
@#else
    @#if CITR ==1
         ...
         save('results1_CITR','CITR_oo_','CITR_case_title');
    @#else
        @#if PEG ==1
             PEG_oo_ = oo_;
             PEG_case_title = case_title;
             save('results1_PEG','PEG_oo_','PEG_case_title');
        @#else
            @#if DITR_F ==1
                 DITR_F_oo_ = oo_;
                 DITR_F_case_title = case_title;
                 save('results1_DITR_F','DITR_F_oo_','DITR_F_case_title');
            @#else
