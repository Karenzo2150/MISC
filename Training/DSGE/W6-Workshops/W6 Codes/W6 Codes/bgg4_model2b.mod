 ...
 * DOLLARIZATION OF THE ENTREPRENEURS' DEBT
...
var c rnom pic lab wr y mcr a g z inv k n ce rk qr sp mrs piw; //20
...
parameters theta_h chi_h theta_f chi_f rho_i phi_pic phi_y phi_dep chi_dbf rho_a rho_g rho_z;
...
rnom = rnomstar + dep(+1) + varrho*bf + chi_dbf*(bf-bf(-1));
...
ce  = n;
// (BGG4.18) spread of real return of capital over the cost of funds depend of the financial position of entrepreneurs. Dollarization
rk(+1) - (rnomstar+dep(+1)-pic(+1)) = FINACC*sp;
...
// (BGG4.27) Entrepreneur's networth evolution. Dollarization
n = FINACC*(K_N*rk - (K_N-1)*(sp(-1) + rnomstar(-1) + dep - pic) + n(-1));
