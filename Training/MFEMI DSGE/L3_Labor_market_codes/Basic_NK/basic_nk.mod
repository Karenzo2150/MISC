...
THETA   = 0.75;
EPSILON = 6;
KAPPA   = ((1-THETA)*(1-BETA*THETA)/THETA)*(1/(1+EPSILON*ALPHA/(1-ALPHA)));
...
%% 3. Marginal cost
MC = (SIGMA+(VARPHI+ALPHA)/(1-ALPHA))*X;
...
(SIGMA+(VARPHI+ALPHA)/(1-ALPHA))*YN = ((1+VARPHI)/(1-ALPHA))*A;
