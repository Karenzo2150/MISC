 ...
 * Adding local consumer pricing for imports and exports
 * Case (c): LCP for imported and exported goods
 * Different case for the monetary/exchange rate policy
 * CITR: CPI Inflation Targeting (1)
 */
...
var c ch cf prh rer rnom pic pih lab rw y mchr a g z cstar varphi gammaH dep bstar chstar istar pistar tb pihx prhx pif prf;
varexo e_a e_g e_z e_cstar e_istar e_pistar e_varphi e_gammaH;

parameters beta_C sigma alpha_c eta_c sigma_L theta_p theta_px theta_pf phi_i phi_pic phi_y eta_star rho_a rho_g rho_z rho_cstar;
...
sigma_L     = 1;
theta_p     = 0.75;
theta_px    = 0.75; //0.0001 or 0.75
theta_pf    = 0.75; //0.0001 or 0.75
phi_pic     = 1.5;
phi_y       = 0.0;
phi_i       = 0.85;
...
ch  = c - eta_c*prh;
//(SOE.2) Domestic demand for F goods.  Modified for LCP (rer by prf)
cf  = c - eta_c*prf;
//(SOE.3) Price level. Modified for LCP (rer by prf)
...
//(SOE.11) Aggregate demand for H goods.Modification to add LCP
y   = (1-alpha_c)*ch + alpha_c*(cstar-eta_star*(prhx));
//(SOE.12) CPI inflation.  Modified for LCP 
prh = prh(-1) + pih - pic;
//(SOE.13) Balance of payments. Modification to add LCP
bstar = 1/(beta_C)*bstar(-1) + alpha_c*(rer+cf) - alpha_c*(prhx+rer + cstar-eta_star*(prhx)) ;
//(SOE.14) Definition of nominal devaluation
dep = rer-rer(-1) + pic - pistar;
//(SOE.15) Foreign demand for H goods. Modification to add LCP
chstar = cstar - eta_star*(prhx);
//(SOE.16) Trade balance. Modification to add LCP
tb = alpha_c*(chstar + prhx - cf);
...
// Export are set in foreign currency and its price face nominal rigidities
//(SOE.25) Phillips curve for H goods exported in foreign currency
pihx = beta_C*pihx(+1) + (1-theta_px)*(1-theta_px*beta_C)/theta_px*(prh-rer-prhx);
//(SOE.26) Evolution relative price of export relative to the foreign price level
prhx = prhx(-1) + pihx - pistar;
//(SOE.27) Phillips curve for imported goods in local economy
pif = beta_C*pif(+1) + (1-theta_pf)*(1-theta_pf*beta_C)/theta_pf*(rer-prf);
//(SOE.28) Evolution relative price of imported goods relative to CPI
prf = prf(-1) + pif - pic;
end;
...
var e_varphi; stderr 0.25;
var e_gammaH; stderr 0.50;
end;
...
@#if DITR == 1
    ...
    save('results4c_DITR','DITR_oo_','DITR_case_title');
@#else
    @#if CITR ==1
         CITR_oo_ = oo_;
         CITR_case_title = case_title;
         save('results4c_CITR','CITR_oo_','CITR_case_title');
    @#else
        @#if PEG ==1
             PEG_oo_ = oo_;
             PEG_case_title = case_title;
             save('results4c_PEG','PEG_oo_','PEG_case_title');
        @#else
            @#if DITR_F ==1
                 DITR_F_oo_ = oo_;
                 DITR_F_case_title = case_title;
                 save('results4c_DITR_F','DITR_F_oo_','DITR_F_case_title');
            @#else
